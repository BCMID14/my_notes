...
export default function notesReducer(state = INITIAL_STATE, action) {
  switch(action.type){
    ...
    case "UPDATENOTE": {
      ...
      const index = newNotesArray.findIndex((obj) => obj.id === newObject.id);
      newNotesArray.splice(index, 1, newObject);
    ...
    case "DELETENOTE": {
      const newNotesArray = [...state.notes].filter(note => note.id !== action.payload);
      return {notes: newNotesArray}
    }
